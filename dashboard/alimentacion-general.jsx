...
// Se leen desde window para no duplicar la fuente de verdad.
function vgGetUsers()  { return window.HL_USERS  || []; }
function vgGetUserById(id) { return window.getUser ? window.getUser(id) : (vgGetUsers().find(u => u.id === id) || vgGetUsers()[0]); }
...
function vgBuildDefaultMealEvents() {
  return [
    ...
    { id: 'm-3', title: 'Cena Andreu',       mealType: 'cena',     day: 0,
      startTime: '20:00', endTime: '20:30',
      visibility: 'personal', assignedTo: ['u-andreu'], recipes: ['r-3'] },
    ...
    { id: 'm-7', title: 'Almuerzo Andreu',   mealType: 'almuerzo', day: 3,
      startTime: '13:00', endTime: '13:30',
      visibility: 'personal', assignedTo: ['u-andreu'], recipes: ['r-6'] },
        ...
        {event.visibility === 'personal' && event.assignedTo?.length > 0 && (
          <div className="flex items-center gap-1.5">
            {event.assignedTo.map(uid => (
              <VgUserAvatar key={uid} user={vgGetUserById(uid)}/>
            ))}
          </div>
        )}
      </div>
...
function VistaGeneralViewImpl({ currentUserId }) {
  const currentUser = vgGetUserById(currentUserId || 'u-andreu');
