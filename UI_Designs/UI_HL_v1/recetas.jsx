...
// ─── Mock de usuarios de la casa (en repo: useHouseUsers + currentUser) ────
const RC_HOUSE_USERS = [
  { id: 'u-annie',  name: 'Annie',  color: 'peach'  },
  { id: 'u-carlos', name: 'Carlos', color: 'sky'    },
  { id: 'u-sofia',  name: 'Sofía',  color: 'butter' },
];
const RC_CURRENT_USER_ID = 'u-annie';
...
function RecetasViewImpl({ ingredients, types }) {
  ...
  const userById = React.useMemo(
    () => Object.fromEntries(RC_HOUSE_USERS.map(u => [u.id, u])),
    []
  );

  // Filter logic — Personal solo visible para su autor (lógica real de Capa 2)
  const visibleRecipes = React.useMemo(
    () => recipes.filter(r => r.visibility === 'familiar' || r.authorId === RC_CURRENT_USER_ID),
    [recipes]
  );
  ...
  const filtered = React.useMemo(() => {
    ...
    if (scope === 'mine')     r = r.filter(x => x.authorId === RC_CURRENT_USER_ID);
  ...
  const counts = React.useMemo(() => ({
    ...
    mine:     visibleRecipes.filter(r => r.authorId === RC_CURRENT_USER_ID).length,
  }), [visibleRecipes]);
  ...
  const me = userById[RC_CURRENT_USER_ID];
  ...
  return (
    <div className="route-fade space-y-5">
      ...
      <RcFormDialog
        ...
        ingredients={ingredients}
        typeById={typeById}
        currentUserId={RC_CURRENT_USER_ID}
